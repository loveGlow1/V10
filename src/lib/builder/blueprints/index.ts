...
/* The same ceiling an edit's conversation is trimmed to. One number, because a
   new page and an edit make the same promise about what is remembered. */
import { MAX_CONTEXT } from "@/lib/builder/brief";
  ...
  if (context.carriedFrom?.trim()) {
    lines.push(
      `- This continues an earlier description in the same conversation: "${context.carriedFrom
        .trim()
        .slice(0, MAX_CONTEXT)}". The brief above is what to build; this is what it refers back to.`,
    );
  }
