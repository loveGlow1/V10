  ...
  return (
    <div className="w-full max-w-[560px] px-4 py-4 lg:pl-5">
      ...
      <div className="relative mt-4">
        <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#8F939A]" />
        <input
          value={query}
          onChange={(event) => setQuery(event.target.value)}
          placeholder="Search"
          aria-label="Search integrations"
          className="h-10 w-full rounded-xl border border-white/[0.09] bg-white/[0.03] pl-9 pr-3 text-sm text-white outline-none placeholder:text-[#8F939A] focus-visible:border-white/25"
        />
      </div>
      ...
      <div className="mt-3 space-y-2">
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-[14px] font-medium text-white">{integration.name}</p>
                <p className="mt-0.5 text-[12px] leading-relaxed text-[#8F939A]">
                  {integration.blurb}
                </p>
              </div>
              ...
              {pending === integration.id ? (
                <button
                  ...
                  className="shrink-0 text-right text-[12px] leading-tight text-[#8F939A] hover:text-white"
                >
                  Connects once
                  <br />
                  building is live
                ...
                <button
                  ...
                  className="flex h-8 shrink-0 items-center gap-1 rounded-lg border border-white/[0.09] px-2.5 text-[13px] text-[#C7CAD0] transition-colors hover:bg-white/[0.05] hover:text-white"
                >
        ...
        {shown.length === 0 && (
          <p className="py-8 text-center text-sm text-[#8F939A]">
