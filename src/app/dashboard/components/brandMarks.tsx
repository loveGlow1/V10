...
/* The real mark of each service the app connects to.

   The paths are the official ones, taken from Simple Icons (simple-icons.org,
   icon data released CC0) and inlined rather than installed: ten glyphs do not
   justify a dependency, and a path in the bundle cannot fail to load the way a
   remote asset can. Each is a single 24x24 path drawn in currentColor, so a mark
   costs nothing beyond the string below.

   `tint` is the brand's own colour, except where that colour is black — GitHub,
...
export function BrandMark({
  ...
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" role="img" aria-label={title} className={className}>
      <path d={path} />
    </svg>
  );
}
